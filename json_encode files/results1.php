...
$response[0]['question_type'] = 'TX';
$query = mysqli_query($link,"SELECT * FROM SURVEY_1_RESULTS WHERE question_id=0 LIMIT 10");
while($row = mysqli_fetch_assoc($query)){
	$response[0]['answers'][] = $row['answer'];
}

//results for Question1
$response[1]['question_type'] = 'MC';
$query2 = mysqli_query($link,"SELECT COUNT(*) AS c FROM `SURVEY_1_RESULTS` WHERE question_id=1 ");
...
$query2 = mysqli_query($link,"SELECT COUNT(*) AS c FROM `SURVEY_1_RESULTS` WHERE question_id=2 ");
$resultTotal = mysqli_fetch_assoc($query2);
...
$query2 = mysqli_query($link,"SELECT COUNT(*) AS c FROM `SURVEY_1_RESULTS` WHERE question_id=3 ");
$resultTotal = mysqli_fetch_assoc($query2);
